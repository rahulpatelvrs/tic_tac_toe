...
function isOnTop(originTowerIndex, disc) {
  ...
  return (
    disc.style.width === towerContent[originTowerIndex][size - 1].style.width
  );
}
...
function isDiscLessThan(currentTowerIndex, disc) {
  ...
  if (!towerContent[currentTowerIndex][size - 1]) {
    ...
    let sizeBottom = towerContent[currentTowerIndex][
      size - 1
    ].style.width.substring(
      0,
      towerContent[currentTowerIndex][size - 1].style.width.indexOf("p")
    );
